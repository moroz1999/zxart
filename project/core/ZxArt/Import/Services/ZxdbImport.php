...
class ZxdbImport extends errorLogger
{
    ...
    protected array $legalStatuses = [
        ...
        'X' => LegalStatus::unknown,
//        '?' => LegalStatus::mia, - mia is disabled for zxdb due to false positives
    ...
    public function importZxdbProds(): bool
    {
        if ($entries = $this->zxdb->table('entries')->select('*')->get()) {
            foreach ($entries as $entry) {
                ...
                if ($entry['genretype_id'] !== null && !isset($this->allowedCategoryIdsMap[$entry['genretype_id']])) {
                ...
                $prodInfo = [
                    ...
                    'id' => $entry['id'],
                    'language' => null,
                    'categories' => $entry['genretype_id'] === null ? null : [$entry['genretype_id']],
                    'images' => [],
                    'labels' => [],
                    'authors' => [],
                    'publishers' => [],

...
Я скину данные софта, сгенерируй из них краткое и сухое описание программы в виде JSON на трех языках eng/rus/spa. 
Отправляю скриншоты из программы, распознай их и используй тексты и информацию с них.";
        if ($hasTextDescriptions) {
            $prompt .= "* Для каждого языка напиши по три-четыре абзаца, используя html теги для форматирования, ЖЕЛАТЕЛЬНО 150 слов";
        } else {
            $prompt .= "* Для каждого языка напиши по одному-два абзаца, используя html теги для форматирования, всё вместе примерно 100 слов";
        }
        $prompt .= "
* Не выдумывай факты, не пиши отсебятину, пересказывай объективно . Текст должен быть объективным, описательным и точным. Это не картотека, нужны реально параграфы описания. ЕСЛИ НЕ ЗНАЕШЬ ЧТО ПИСАТЬ - НАПИШИ МЕНЬШЕ АБЗАЦЕЙ. 
* НЕ ИСПОЛЬЗУЙ эпитеты \"потрясающий\", \"захватывающий\" , \"особый\", \"уникальный\" итд. Пиши по существу и фактам.
...
* Не пиши противоречивые фразы вроде 'простой, но сложный геймплей'.
* Не пиши про игры 'предлагает сложный опыт' или 'уникальный опыт' или 'ностальгический опыт' - откуда ты знаешь, какой там опыт? 
* Не пиши банальные фразы типа 'графика характерна для того времени'. Это и так ясно посетителю, он видит год публикации. 
* Не пиши общие фразы про то, что игра 'требует быструю реакцию и стратегическое мышление'. Используй скриншоты и описывай сеттинг в общих чертах.
* Не пиши про 'увлекательный игровой процесс', если ты не знаешь наверняка. Если это известная игра типа manic miner, то да, можно писать про её игровой процесс. А если у тебя НЕТ информации про неё в базе, то выдумывать нельзя.
* Пиши только полезное читателю, не надо 'воды' типа 'уникальные испытания и окружения' или 'Игроки проходят сложные уровни, преодолевая препятствия и выполняя точные прыжки для продвижения'. Пиши ТОЛЬКО САМУЮ конкретику.
* Party в описаниях демо - это в смысле demoparty или competition.
* Gift переводи как гифт.
* Постарайся не перечислять создателей, это малоприоритетная информацию, её люди видят и так.

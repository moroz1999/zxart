...
Есть сайт-коллекция софта для ZX Spectrum, Sam Coupe, ZX Next, ZX81, ZX Evolution и тд, нужно сделать SEO, чтобы люди в поисковике нашли нужную информацию. 
...
* h1 (120 символов) будет показан посетителю уже на сайте, это главный заголовок над текстом. Он должен быть человекопонятным и сразу дать кратко понять, что это за программа. Например, по-русски это был бы 'Экшен-платформер игра \"Captain Square Jaw\" (2024)', а по-английски что-то вроде 'Action Platformer game \"Captain Square Jaw\" (2024)'. Не используй оценочных суждений типа 'Впечатляющий', 'Увлекательный' итд
* 'Ощутите ностальгию и захватывающий игровой процесс' - это ОЧЕНЬ плохой пример. Нельзя писать про ретро, ностальгию. Нельзя писать про захватывающий игровой процесс, если не знаешь 100% точно что он захватывающий.   
* Не используй идиотских call to action типа 'Откройте для себя' - это не йогурт, это программы для ретро-компьютеров.
* В Meta description нужно уместить краткое описание, увидев которого человек захотел бы кликнуть и посмотреть страницу среди других страниц в результатах поиска. Нужно выгодно выделиться информативностью и ценными фактами, а не эпитетами. Желательно указать фразу про что программа, год, издателя, жанр. По возможности нужно использовать все 170 символов под завязку.
* Eng поля должны быть на английском языке, Rus - на русском, Spa - на испанском.
* Party в описаниях демо - это в смысле demoparty или competition.
* Gift переводи как гифт.  
...
";
    ...
    public function querySeoForProd(zxProdElement $prodElement): ?array
    {
        ...
        $imageUrls = $prodElement->getImagesDownloadUrls();
...
Я скину данные софта, сгенерируй описание программы в виде JSON на трех языках eng/rus/spa. 
...
* Для каждого языка напиши от одного до четырёх абзацей текста. Если информации много (есть инструкция, сюжет, скриншоты), то напиши больше текста. Если писать не о чем, то меньше текста.
* Стиль - сухой, БЕЗ эпитетов. 
* Не выдумывай факты, не пиши отсебятину, пересказывай объективно. 
* НЕ ИСПОЛЬЗУЙ оценочные эпитеты. Описывай имеющиеся факты, сюжет, суть.
* Не пиши про управление и hardware.
* Больше всего пиши про сюжет (если это игра или демка), а ещё пиши про жанр и особенности программы.  
* В системном и прикладном софте пиши про функционал.
...
* Не пиши про то, что это ретро-программы. Вообще это не упоминай.  
* Party в описаниях демо - это в смысле demoparty или competition.
* Gift переводи как гифт.
* Не перечисляй отдельных авторов, ЭТО НЕ НУЖНО.
* В ответе не пиши ни слова, ТОЛЬКО JSON в формате:
* НЕ делай выводов о качестве, значимости, уровне, навыках, духе, вкладе, сотрудничестве, ценности или назначении произведения.
* Описывай ТОЛЬКО то, что непосредственно видно, указано в данных или явно следует из текста/скриншотов (сцены, сюжет).
* Не перечисляй напрямую объекты со скриншотов. Не упоминай интерфейс, надписи на скринах. Используй скриншоты для более обобщённой информации о сеттинге, жанре или тематике. 
{
eng:'all text here',
rus:'',
spa:''

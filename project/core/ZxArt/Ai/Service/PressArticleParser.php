...
readonly class PressArticleParser
{
    ...
    public function getParsedData(string $text, int $articleId, string $pressTitle, ?int $pressYear): ?array
    {
        $createPrompt = static function (string $chunk) use ($pressTitle, $pressYear): string {
            return "Отправляю тебе через API текст статьи из журнала/газеты для ZX Spectrum. Сделай всё правильно с первой попытки, так как я не смогу проверить и указать на ошибки. 
* В persons собери всех людей из других полей в формате полного объекта AuthorObj.
    ...
    Не сочиняй realName, оставь пустым, если есть сомнения. Укажи его в тех случаях, когда имя и фамилия выглядят как реальные и общепринятые имена. Если псевдоним не соответствует этим критериям, оставляй поле пустым. Псевдонимы в поле realName использовать не следует.
    Если известно полное имя и фамилия, то используй формат \"Имя Фамилия\" в полной форме - например, вместо \"sasha\" пиши \"Александр\", вместо Misha - Михаил, вместо Макс - Максим итд. Валера - Валерий. Коля - Николай. 
    Если написана фамилия, а потом имя, то переставь в формат Имя Фамилия. Например, 'Popirin Denis' меняй на 'Денис Попырин'. 'Sobolev E.A' меняй на 'Е. Соболев'.
    Если имя человека сокращенное и неизвестно точно какое (Например, 'А. Иванов' или 'Иванов А.'), то НЕ ВЫДУМЫВАЙ Александр или Алексей - просто оставь как есть А. Иванов.
    НЕ ПИШИ ОТЧЕСТВО.
    ...
    Имя автора должно быть в именительном падеже. Вместо 'Пола Андерсона' пиши 'Пол Андерсон'.
...
Люди в статье иногда указаны вместе с командой. MyAuthor^MyTeam означает, что автор состоит напрямую в MyTeam. Если роль не указана явно, не добавляй поле teamRoles.
...
* Если в статье ЯВНО указано, кто написал текст статьи, то собери их id в поле articleAuthorIds. Если nickname нет, то укажи реальное имя. Если в статье в самом начале отдельно написано имя автора с (C), то это автор статьи. Например, (C) Welcome & TP 1997 - авторы статьи Welcome и TP, написано в 1997 году. Если в конце указано что-то типа M.M.A'96 , то это означает, что статью писал M.M.A в 1996 году, УКАЖИ ЕГО В АВТОРАХ СТАТЬИ.
* Если в статье указаны авторы всего ЖУРНАЛА или ГАЗЕТЫ, то собери их id и роли (если есть) в pressAuthorship в формате AuthorshipObj и SoftwareRole. 
    Если в статье указан автор музыки к статье, добавь его в авторы издания в pressAuthorship. 
    Музыку всегда пишет человек, не команда. Music by Midisoft в начале статьи означает, что автор музыки Midisoft, и это человек, а не команда, добавь его в авторы издания в pressAuthorship.
    Если редакция упоминает авторов статей (например, в благодарностях, списке участников редакции, либо как авторов статей или материалов), то добавь их в авторы издания в pressAuthorship.  
* Если в отдельной статье об авторах, от авторов (или Credits) указана команда, выпустившая САМ ЖУРНАЛ или ГАЗЕТУ, то положи её id в pressTeams. Не указывай, если нет явного упоминания. 
...
* Собери ВСЕ упомянутые системные программы, игры, демо, издания, журналы и газеты в поле software. Музыкальные ТРЕКИ и картинки с конкурсов НЕ ОТНОСЯТСЯ к software. Все упомянутые журналы и газеты - электронные, поэтому они относятся к software.
    ...
    В названии журналов и газет МОЖЕТ быть номер, сохраняй номер если он есть. Если номер меньше 10, то надо писать двузначный #01, #02 итд. Например, Adventurer #03 - это полное и правильное название конкретного журнала.
...
* Если в статье идёт речь об игре или программе, то самый первый заголовок - это её название и есть.  
* Не переводи названия программ и групп, оставь их так, как упомянуты в статье. 'Звёздное наследие' оставь как 'Звёздное наследие', НЕ ПЕРЕВОДИ в 'Star Inheritance'. 
* Названия программ и групп переделывай из капслока в нормальный регистр, если это не аббревиатуры. Например, PSG - похоже на аббревиатуру, оставь капслок. DREAM TEAM - явно не аббревиатура, переделай в Dream Team.
...
* Игнорируй журнал ZX Ревю до 1997 года выпуска.
* Убирай из названия программ версии под память 128 и 48. Например 'Empire 128' должна стать просто 'Empire'.
* Убирай из названия программ количество уровней. Например 'Dark Fusion +4 levels' должна стать просто 'Dark Fusion'.
...
* Если встретишь упоминания этих названий, то это точно persons: KSA, Midisoft, IMP,  nix./site, M.M.A
* Типы полей:

...
readonly class AuthorListService
{
    ...
    public function getPaged(
        ...
        $total = $this->repository->count($search, $countryId, $cityId, $letter, $types);
        $ids = $this->repository->findPaged($start, $limit, $sorting->column, $sorting->direction, $search, $countryId, $cityId, $letter, $types);
     ...
     */
    public function getFilterOptions(?string $letter = null): array
    {
        $countryIds = array_unique($this->repository->findCountryIds($letter));
        $cityIds = array_unique($this->repository->findCityIds($letter));

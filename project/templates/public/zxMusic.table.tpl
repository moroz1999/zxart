<tr class="">
	...
	<td class='music_list_download'>
		...
		{if $element->getMp3FilePath()}<a rel="nofollow" href="{$element->getMp3FilePath()}"><img loading="lazy" class="music_list_mp3" src="{$theme->getImageUrl("ogg.svg")}" alt="{translations name='label.download'} {$element->getFileName('mp3', false)}" /> </a>{/if}
	</td>
</tr>
<script>
	{if $isPlayable}

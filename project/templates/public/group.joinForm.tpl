...
<form action="{$element->URL}" method="post" class="group_form" enctype="multipart/form-data">
	<table class='form_table'>
		<tr class="{if $formErrors.joinAsAlias} form_error{/if}">
			<td class="form_label">
				{translations name='group.joinasalias'}:
			</td>
			<td class="form_field">
				<select class="group_form_joinasalias_select" name="{$formNames.joinAsAlias}" autocomplete='off'></select>
			</td>
		</tr>
		<tr class="{if $formErrors.joinAndDelete} form_error{/if}">
			<td class="form_label">
				{translations name='group.joinanddelete'}:
			</td>
			<td class="form_field">

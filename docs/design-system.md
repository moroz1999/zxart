...
5. **No Hardcoded Palette in Components**: In component styles, it is forbidden to use base palette variables like `--gray8` directly. Instead, use semantic variables like `--text-color`, `--zx-button-bg`, etc. If a needed semantic variable is missing, define it in the theme.

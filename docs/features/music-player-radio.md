# Music player and radio — implementation

Domain rules: [../domain/music-player-radio.md](../domain/music-player-radio.md).
...
- `demoscene`: prod category = Demoscene (`CategoryIds::DEMOSCENE`, including subcategories) + min rating = averageVote + 0.2
- `press`: prod category = Press (including subcategories) + min rating = averageVote + 0.2
...
- `prodCategoriesInclude` uses zxProd categories (Press/Games/Demoscene) and expands to the full subcategory tree before filtering tunes by linked prods. `RadioOptionsService` offers the same three as the filter dropdown. Expansion goes through `ProdCategoryTreeService`; the roots are the top-level sections (`CategoryIds::GAMES`, `DEMOSCENE`, `PRESS`), never the leaves inside them.
- `minPartyPlace` is optional; values <= 0 disable the party place filter.
- `requireProd` restricts to tunes that belong to a production at all (`module_zxmusic.prod`); no preset sets it.

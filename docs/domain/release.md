...
and CP/M disks, IS-DOS floppies, Opus Discovery disks, DISCiPLE and +D disks,
SAM Coupé disks, Didaktik D40 and D80 disks, TAP and TZX tapes, TAR distribution
trees, and ordinary ZIP and RAR archives. The systems that keep real folders — an
IS-DOS disk, a TAR tree — are listed with them; the flat ones list everything at
once. A container nobody recognises stays a single file. Names are shown as the
machine that wrote them would have shown them, Russian catalogues included, and a
tokenized BASIC program is previewed as a listing whichever system saved it.

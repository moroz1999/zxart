...
- For new elements that are not yet saved to the database, the `id` property is not `null`. It is a synthetic structure-path string ending in `type:{type}/action:{action}`. To check if an element is already persisted in the database, use `$element->hasActualStructureInfo()`.
- Use `$element->getIdentifier()` when code must preserve a transient synthetic ID. `$element->getId()` returns the numeric persisted ID and therefore evaluates a transient ID as `0`.
- An element method that reads records keyed by the element ID must therefore refuse a transient element and answer with nothing: `getId()` gives `0`, and a query built from the synthetic string makes the database cast it to `0` as well, so the read returns whatever rows an old import left under ID 0. `hasActualStructureInfo()` is the check — authorship, import origins and queue records all make it. Link-backed reads through `linksManager::getConnectedIdList()` need no check: they compare each link's ID against the synthetic string, which matches nothing.
...
- `structureElement::getFormData()` hands out raw chunk form values, and the chunk classes are legacy: their types are fixed and MUST NOT be changed to suit one consumer. Most chunks (`text`, `html`, `textarea`, `url`, `code`, `email`, `structureName`, `password`, `checkbox`, `naturalNumber`, `image`, `file`, `array`, `numbersArray`) pass the stored value through as the DB row gives it — a string, an array for the array chunks — or `null` when nothing is stored, which is the case for every field of a creation draft. The converting ones: `date`, `dateTime`, `time` → formatted string or `''`; `money`, `floatNumber` → float; `emptyMoney` → `''` or float; `serializedArray`, `serializedIndex`, `jsonSerialized` → array or `null`; `ConnectedElements` → list of elements. Multi-language fields arrive as `{languageId: value}` maps of those same types; `getMultiLanguageFields()` says which fields those are.
- Consumers must account for those types at the point of use, cast included. In particular, a `null` sent to a browser as a form value returns as the literal string `"null"` (`FormData.append()` stringifies it), so JSON form endpoints cast text values to string — `/formdata/` does it for multi-language fields.

...
- **Architecture**:
    ...
    - **Negative margins are PROHIBITED**. Never use `calc(-1 * ...)` or any negative margin trick to break out of a parent's padding. Instead, use `padding="none"` on the parent (e.g., `zx-panel`) and apply padding to the child elements that need it.

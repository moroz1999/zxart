...
@Component({
  selector: 'zx-party-ratings',
  standalone: true,
  imports: [CommonModule, TranslateModule, ZxWorksRatingsPanelComponent],
  template: `
    <zx-works-ratings-panel
      [title]="'party-details.activity.votes' | translate"
      [loader]="loader">
    </zx-works-ratings-panel>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ZxPartyRatingsComponent {
  @Input() elementId = 0;

  constructor(private readonly ratingsService: RatingsService) {}

  loader = (page: number): Observable<AuthorRatingsListDto> =>
    this.ratingsService.getPartyRatings(this.elementId, page);
}

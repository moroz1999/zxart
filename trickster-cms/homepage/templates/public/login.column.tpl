	...
	<div class='login_short'>
		...
		{else}
			<div class="login_component{if $formErrors.userName || $formErrors.password} form_error{/if}">
				...
				{if $socialPlugins}
					{stripdomspaces}
					<div class="login_socialplugins">
						<div class="login_socialplugins_title">
						{translations name='login.connect_with_social_plugins'}:
						</div>
						<div class="login_socialplugins_list">
						{foreach $socialPlugins as $plugin}
							<a class="login_socialplugin login_socialplugin_{$plugin.code}" href="{$plugin.url}" title="{$plugin.title}">
								{if $plugin.icon}
									<img src="{$plugin.icon}" class="login_socialplugin_icon" alt="{$plugin.title}"/>
								{else}
									<span class="login_socialplugin_text">{$plugin.title}</span>
								{/if}
							</a>
						{/foreach}
						</div>

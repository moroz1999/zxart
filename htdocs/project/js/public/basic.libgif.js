(function(window) {
	window.AnimatedGifParser = function() {
		var stream;
		var hdr;
		...
		var canvas, ctx;
		var parseCompleteCallback;

		var gifWidth;
		var gifHeight;

		this.startParsing = function(gifBody, callback) {
			if (typeof callback == "function") {
				parseCompleteCallback = callback;
			}
			init();
			stream = new Stream(gifBody);
			setTimeout(doParse, 0);
		};

		this.getGifData = function() {
			return compileGifData();
		};

		var compileGifData = function() {
			return {
				'width': gifWidth,
				'height': gifHeight,
				'frames': frames
		...
		var handler = {
			...
			eof: function() {
				...
				if (parseCompleteCallback) {
					parseCompleteCallback(compileGifData());
				}
			}
		};

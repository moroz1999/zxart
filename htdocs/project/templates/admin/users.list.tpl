...
{if $contentList}
    <table class='content_list'>
        ...
        <tr>
            ...
            <th class="verified_column">
                Verified
            </th>
            <th class="comments_column">
                Commenting
            </th>
            <th class="banned_column">
                Banned
            </th>
            ...
            <tr class="content_list_item elementid_{$contentItem->id}">
                <td class="checkbox_column">
                    <input class='singlebox checkbox_placeholder' type="checkbox"
                           name="{$formNames.elements}[{$contentItem->id}]" value="1"/>
                </td>
                {if method_exists ($contentItem, 'getImageUrl')}
                    <td class='image_column'>
                        {if $contentItem->image}
                            <img loading="lazy" src='{$contentItem->getImageUrl()}' alt=" "/>
                        {/if}
                    </td>
                {/if}
                ...
                <td class="verified_column">
                    {$contentItem->verified}
                </td>
                <td class="comments_column">
                    {$contentItem->comments}
                </td>
                <td class="banned_column">
                    {$contentItem->banned}
                </td>
                <td class='edit_column'>
                    {if isset($privilege.showForm) && $privilege.showForm}
                        <a href="{$contentItem->URL}id:{$contentItem->id}/action:showForm"
                           class='icon icon_edit'></a>
                    {/if}
                </td>
                ...
                <td class="delete_column">
                    {if isset($privilege.delete) && $privilege.delete}
                        <a href="{$contentItem->URL}id:{$contentItem->id}/action:delete"
                           class='icon icon_delete content_item_delete_button'></a>
                    {/if}
                </td>
            </tr>
        {/foreach}

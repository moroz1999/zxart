...
<form action="{$element->URL}" method="post" class="zxitem_form" enctype="multipart/form-data">
    <table class='form_table'>
        ...
        <tr {if $formErrors.categories} class="form_error"{/if}>
            ...
            <td class="form_field">
                <select class="zxitem_form_categories_select select_multiple" name="{$formNames.categories}[]"
            ...
            <td class="form_field">
                <select class="select_multiple" name="{$formNames.language}[]" multiple="multiple">
                    {foreach from=$element->getLanguageCodes() item=languageItem}
                        <option value='{$languageItem}'
                                {if in_array($languageItem, $formData.language)}selected='selected'{/if}>{translations name="language.item_{$languageItem}"}</option>
                    {/foreach}
                </select>
            ...
            <td class="form_field">
                <select class="dropdown_placeholder" name="{$formNames.legalStatus}">
                    <option value='unknown'
                            {if $formData.legalStatus=='unknown'}selected='selected'{/if}>{translations name='legalstatus.unknown'}</option>
                    <option value='allowed'
                            {if $formData.legalStatus=='allowed'}selected='selected'{/if}>{translations name='legalstatus.allowed'}</option>
                    <option value='allowedzxart'
                            {if $formData.legalStatus=='allowedzxart'}selected='selected'{/if}>{translations name='legalstatus.allowedzxart'}</option>
                    <option value='forbidden'
                            {if $formData.legalStatus=='forbidden'}selected='selected'{/if}>{translations name='legalstatus.forbidden'}</option>
                    <option value='forbiddenzxart'
                            {if $formData.legalStatus=='forbiddenzxart'}selected='selected'{/if}>{translations name='legalstatus.forbiddenzxart'}</option>
                    <option value='insales'
                            {if $formData.legalStatus=='insales'}selected='selected'{/if}>{translations name='legalstatus.insales'}</option>
                    <option value='mia'
                            {if $formData.legalStatus=='mia'}selected='selected'{/if}>{translations name='legalstatus.mia'}</option>
        ...
        <tr {if $formErrors.publishers} class="form_error"{/if}>
            ...
            <td class="form_field">
                <select class="select_multiple zxitem_form_publishers_select" multiple="multiple"
                        name="{$formNames.publishers}[]" autocomplete='off'>
                    <option value=''></option>
        ...
        <tr {if $formErrors.partyplace} class="form_error"{/if}>
            ...
            <td class="form_field">
                <input class='input_component' type="text" value="{$formData.partyplace}"
                       name="{$formNames.partyplace}"/>
        ...
        <tr>
            ...
            <td class="form_field">
                <input class='checkbox_placeholder' type="checkbox" value="1"
                       name="{$formNames.denyVoting}"{if $element->denyVoting} checked="checked"{/if}/>
        ...
        <tr>
            ...
            <td class="form_field">
                <input class='checkbox_placeholder' type="checkbox" value="1"
                       name="{$formNames.denyComments}"{if $element->denyComments} checked="checked"{/if}/>
        ...
        {include file=$theme->template('component.form.authors.tpl') element=$element displayDate=false type='prod' translationsGroup=$element->structureType}
        ...
        {include file=$theme->template('component.form.files.tpl') element=$element propertyName='connectedFile' imagePreset='prodImage'}
